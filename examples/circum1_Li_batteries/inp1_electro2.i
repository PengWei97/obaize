...
[BCs]
  ...
  # 6) theta_m on internal electrode/electrolyte interface:
  #    Faradaic coupling:  j_Li · n = i_n / F = -sigma * (∇phi · n) / F
  #    implemented in the custom IntegratedBC ThetaFaradaicBC.
  [./theta_faradaic_interface]
    ...
    # from electrode to electrolyte; set to -1.0 if you ever need
    # to flip the sign.
    normal_sign  = 1.0
  [../]
[]
...
[Materials]
  ...
  [./pf_conc_diffusivity]
    ...
    expression = 'D_eff / max( min(1.0, 1.0 - theta_m), 1e-6 )'
